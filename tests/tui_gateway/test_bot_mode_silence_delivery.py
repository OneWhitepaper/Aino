...
def _turn(result):
    return SimpleNamespace(
        ...
        error_detail="", prompt_text="ping", metrics_start=None,
    )
...
def test_live_bot_chat_completion_empties_marker_only_for_successful_turns(monkeypatch):
    ...
    payload, _, status = srv._complete_turn_payload(session, _turn({"final_response": " *NO_REPLY* "}), None, 80, "sid")
    assert (status, payload["text"]) == ("complete", "")

    prose = "[SILENT] is mentioned here, but this is a real answer."
    payload, _, _ = srv._complete_turn_payload(session, _turn({"final_response": prose}), None, 80, "sid")
    assert payload["text"] == prose

    failed = {"final_response": "NO_REPLY", "error": "provider failed", "failed": True}
    payload, _, status = srv._complete_turn_payload(session, _turn(failed), None, 80, "sid")
    ...
    payload, _, _ = srv._complete_turn_payload(session, _turn({"final_response": "NO_REPLY"}), None, 80, "sid")
