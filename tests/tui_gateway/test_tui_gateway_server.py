...
def test_prompt_submit_rebind_map_preserves_proven_physical_rows_after_sequence_repair(
    monkeypatch, tmp_path
):
    """An exact durable replay restores physical boundaries before rewinding."""
    from hermes_state import SessionDB
    ...
    try:
        ...
        assert row_id_map == {
            **{str(row_id): row_id for row_id in physical_ids[:3]},
            **{str(row_id): None for row_id in physical_ids[3:]},
        }
        assert [(m["_row_id"], m["content"]) for m in sess["history"]] == [
            (m["_row_id"], m["content"]) for m in physical[:3]
        ]
        assert "999999" not in row_id_map
    finally:
        server._sessions.pop(sid, None)
