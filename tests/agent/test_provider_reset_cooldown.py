...
def _offline_model_metadata(monkeypatch):
    # These tests exercise cooldown routing, not the remote Codex model catalog.
...
def test_fallback_switch_benches_primary_until_valid_future_provider_reset(reset_at, expected_seconds):
    ...
    with (
        ...
        patch("agent.fallback_cooldown.time", SimpleNamespace(time=lambda: 1_700_000_000, monotonic=lambda: 500)),
    ):
