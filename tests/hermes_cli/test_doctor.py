...
class TestMacOSTCCGrants:
    ...
    def test_desktop_bundle_selection_prefers_aino_and_falls_back_to_hermes(self, tmp_path):
        ...
        assert doctor_platform._select_desktop_app_bundle(release) == aino
        aino.rmdir()
        assert doctor_platform._select_desktop_app_bundle(release) == legacy
    ...
    def test_aino_tcc_guidance_uses_aino_bundle_identifier(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(doctor_platform.sys, "platform", "darwin")
        app = tmp_path / "Aino.app"
        monkeypatch.setattr(doctor_platform, "_desktop_app_bundle", lambda: app)
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
