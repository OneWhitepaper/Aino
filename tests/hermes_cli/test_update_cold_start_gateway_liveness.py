...
def _run_cold_start(monkeypatch, capsys, *, surviving_pids, desktop_owned=False):
    monkeypatch.setattr(update_cmd, "_desktop_owns_gateway_lifecycle", lambda: desktop_owned)
...
def test_failed_readiness_keeps_the_dead_attestation_for_the_retry(monkeypatch, tmp_path, capsys):
    ...
    with pytest.raises(RuntimeError, match="did not become ready"):
        _run_cold_start(monkeypatch, capsys, surviving_pids=[], desktop_owned=True)
