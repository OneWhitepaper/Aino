...
export class JsonRpcGatewayClient {
  ...
  async connect(wsUrl: string): Promise<void> {
    ...
    socket.addEventListener('close', event => {
      ...
      this.dropSocket(new Error(resolveGatewayErrorMessage(this.options.closedErrorMessage)))
    })
  ...
  private connectFailure(detail: string): Error {
    return new Error(`${resolveGatewayErrorMessage(this.options.connectErrorMessage)} (${detail})`)
  }
  ...
  invalidate(message = this.options.closedErrorMessage): void {
    ...
    this.dropSocket(new Error(resolveGatewayErrorMessage(message)))
  ...
  request<T>(
    ...
    return this.channel.request<T>(method, params, timeoutMs, signal, () => new Error(resolveGatewayErrorMessage(this.options.notConnectedErrorMessage)))
  }
