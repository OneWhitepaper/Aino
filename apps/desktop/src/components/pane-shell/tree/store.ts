...
/** Whether a physical side column is fully on screen (rather than hidden or
...
export function isTreeSideVisible(side: TreeSide): boolean {
  ...
  return row.children.some(
    child => paneRootSide(allPaneIds(child)[0]) === side && allPaneIds(child).some(paneId => isPaneVisible(paneId))
  )
}
