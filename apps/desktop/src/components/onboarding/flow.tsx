    ...
    >
      {[...code].map((ch, i) =>
        ch === '-' || ch === ' ' ? (
          ...
          <span
            className={cn(
              'flex size-10 items-center justify-center rounded-(--aino-radius-control) border font-mono text-xl font-semibold uppercase transition-colors',
              copied
...
function CodeBlock({ copied, onCopy, text }: { copied: boolean; onCopy: () => void; text: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-(--aino-radius-control) border border-(--stroke-nous) px-3 py-2">
