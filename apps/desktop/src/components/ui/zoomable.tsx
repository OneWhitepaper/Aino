...
export function Zoomable({ children, overlay, onCopy, label, className }: ZoomableProps) {
  const { t } = useI18n()
  const resolvedLabel = label ?? t.ui.accessibility.openFullView
...
function Toolbar({
  ...
  return (
    <div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 items-center gap-1 rounded-full border border-border/70 bg-background/85 p-1 shadow-sm backdrop-blur">
      <ToolbarButton label={t.ui.accessibility.zoomOut} onClick={zoomOut}>
        <ZoomOut className="size-4" />
      </ToolbarButton>
      <ToolbarButton label={t.ui.accessibility.resetZoom} onClick={reset}>
        <RefreshCw className="size-4" />
      </ToolbarButton>
      <ToolbarButton label={t.ui.accessibility.zoomIn} onClick={zoomIn}>
      ...
      {onCopy && (
        <>
          <Divider />
          <ToolbarButton
            label={copied ? t.ui.accessibility.copied : t.ui.accessibility.copy}
            onClick={() => void copy()}
          >
      ...
      <Divider />
      <ToolbarButton label={t.ui.accessibility.close} onClick={onClose}>
        <X className="size-4" />
      </ToolbarButton>
    </div>
  )
