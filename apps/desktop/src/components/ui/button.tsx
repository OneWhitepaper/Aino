...
// Controls share the chat surface's soft corners and quiet graphite actions.
// Padding-driven sizes still allow labels to scale without fixed-height clipping.
const buttonVariants = cva(
  "inline-flex shrink-0 cursor-pointer items-center justify-center gap-1.5 rounded-(--aino-radius-control) text-[length:var(--aino-text-ui)] leading-4 font-medium whitespace-nowrap shadow-none transition-colors duration-100 outline-none focus-visible:border-ring focus-visible:ring-[0.1875rem] focus-visible:ring-ring/50 disabled:pointer-events-none disabled:cursor-default disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5",
  {
    variants: {
      variant: {
        default: 'bg-(--aino-action-bg) text-(--aino-action-fg) hover:bg-(--aino-action-hover)',
        destructive:
          'bg-destructive text-white hover:bg-destructive/90 focus-visible:ring-destructive/20 dark:bg-destructive/60 dark:focus-visible:ring-destructive/40',
        // Quiet action — transparent fill with a 1.5px inset ring (no layout-shifting border).
        outline:
          'bg-transparent text-(--ui-text-primary) shadow-[inset_0_0_0_1px_var(--ui-stroke-secondary)] hover:bg-(--chrome-action-hover) hover:text-(--ui-text-primary)',
        ...
        text: `rounded-none text-muted-foreground underline-offset-4 hover:text-foreground hover:underline ${TEXT_ACTION_ICON}`,
        // Emphasized inline-text action: bold + always-underlined link. Use for
        // the actionable affordance in a row ("Change", "Set", "Open logs", …).
        textStrong: `rounded-none font-semibold text-muted-foreground underline underline-offset-4 hover:text-foreground ${TEXT_ACTION_ICON}`
      },
      size: {
