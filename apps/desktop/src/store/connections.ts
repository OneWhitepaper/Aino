...
// Matches the primary cold-start budget (port announcement + health + final
// handshake); anything longer means the primary is not coming and the registry
// restore should stop waiting for it. Shared constant so boot-class budgets
// can't drift apart (see with-timeout.ts).
