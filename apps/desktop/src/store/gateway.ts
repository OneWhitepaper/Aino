...
/** Tag a primary-socket request with its source, even while another source is active. */
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  ...
  entry.offRequest =
    gateway.onRequest?.(request => {
      ...
      return dispatchServerRequest(request, profile, connectionId)
    }) ?? (() => {})
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.hermesDesktop?.getConnectionFor) {
    throw new Error(translateNow('settings.gateway.registryConnectionsUnsupported'))
  }
