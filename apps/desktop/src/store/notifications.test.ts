...
test('gateway_auth_failed error is summarized as sign-in, with an Open Gateways action', () => {
  ...
  expect(action?.label).toBe(translateNow('notifications.actions.openGateways'))
  action?.onClick()
...
test('405 method-not-allowed toasts a restart in plain words with a restart action', () => {
  ...
  expect($notifications.get()[0]?.action?.label).toBe(translateNow('notifications.actions.restartHermes'))
  $notifications.get()[0]?.action?.onClick()
