...
function appendPreviewArtifact(
  sid: string,
  raw: string,
  cwd: string,
  id: string,
  ...
  writePreviews(
    sid,
    [...list, { cwd, generated, id, label: previewName(raw), target: raw, dismissalScope: scope }].slice(
      -MAX_PER_SESSION
    )
  )
}
...
export function recordPreviewArtifact(
  ...
  target: string,
  cwd: string,
  dismissalSid: string | boolean = sid,
  generated = false
) {
  ...
  appendPreviewArtifact(
    ...
    cwd,
    id,
    reconcileDismissalScope(sid, typeof dismissalSid === 'string' ? dismissalSid : sid),
    typeof dismissalSid === 'boolean' ? dismissalSid : generated
  )
}
...
export function reofferPreviewArtifact(sid: string, target: string, cwd: string, dismissalSid = sid) {
  ...
  appendPreviewArtifact(sid, raw, cwd, id, scope, true)
}
