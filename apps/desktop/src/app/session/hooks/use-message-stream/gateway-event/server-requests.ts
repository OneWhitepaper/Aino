...
const approval: Handler = ctx => {
  ...
  const description = str(p.description) || translateNow('prompts.dangerousCommand')
...
const tour: Handler = ({ isActiveSession, request }) => {
  ...
  if (!$toursEnabled.get()) {
    ...
    answerValue(request, { error: translateNow('preview.tour.disabled'), success: false })

    return
  }

  if (!isActiveSession) {
    answerValue(request, { error: translateNow('preview.tour.inactiveSession'), success: false })
