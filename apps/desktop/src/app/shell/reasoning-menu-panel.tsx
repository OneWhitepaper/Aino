...
export function ReasoningMenuPanel(props: ModelMenuHostProps) {
  ...
  const { activePlatformModel, controller, defaultEffort, modelOptions } = useModelMenuController(props)
  const { model, provider } = controller.current
  const caps = currentModelCapabilities(modelOptions.data, provider, model)
  const managed = provider === 'aino'

  if (!model || (managed && !activePlatformModel?.capabilities.reasoning)) {
    return (
