...
// including the model and approval icons plus the queue button mid-turn.
//
// Below 300 the voice controls fold into the menu HUD mode already uses,
// keeping the added approval icon from crowding Send. Each stage sits clear
...
export const COMPOSER_FOLD_VOICE_PX = 300

// Type and send, nothing else. A pane can be dragged to MIN_PANE_PX (80), and
// even with voice folded the row still costs ~196px before surface padding
// (including approval and queue), so the last rung drops secondary controls
// below 240px. The model stays reachable by hotkey and the
// full picker, dictation from any wider pane — and Send fits with room to
// spare at any width the layout tree allows (~74 all-in).
export const COMPOSER_MINIMAL_PX = 240
