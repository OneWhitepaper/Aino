...
const openExternalUrl = createExternalUrlOpener({ shell, isWsl: IS_WSL, log: rememberLog })
...
async function waitForUpdateToFinish() {
  ...
  try {
    notifyHandoffResult(readAndConsumeHandoffResult(HERMES_HOME), {
      ...
      retryUpdate: sendOpenUpdatesRequested,
      revealLog: () => shell.showItemInFolder(path.join(HERMES_HOME, 'logs', 'desktop-update-handoff.log'))
    })
  } catch (err) {
...
const platformCaptcha = createPlatformCaptcha({
  ...
  generation: () => platformAuth.generation(),
...
const platformIpc = registerPlatformIpc({
  ...
  currentDeviceId: () => desktopInstallationId,
  fromWebContents: sender => BrowserWindow.fromWebContents(sender as Electron.WebContents),
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  win.webContents.on('will-navigate', (event, url) => {
    ...
    event.preventDefault()
    void openExternalUrl(url).catch(error => rememberLog(`[link] openExternal failed: ${describeCrashReason(error)}`))
  })
}
