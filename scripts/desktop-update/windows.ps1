...
try {
    ...
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Aino install open ($shim locked after 20s). Nothing was changed. Close other Aino windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
