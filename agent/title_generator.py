...
def start_title_upgrade(upgrade: Optional[threading.Thread]) -> None:
    ...
    tracker = getattr(upgrade, "_managed_billing_tracker", None)
    if tracker:
...
def maybe_auto_title(
    ...
    def generate_in_background():
        try:
            auto_title_session(session_db, session_id, user_message, **upgrade_kwargs)
        finally:
            if tracker:
                tracker.end_background()

    upgrade = spawn_context_thread(generate_in_background, name="auto-title")
    upgrade._managed_billing_tracker = tracker
