...
def _attach_session_transport(session: dict | None, transport) -> bool:
    ...
    with _session_transport_lock:
        ...
        if session.get("managed_model_params"):
            ...
            if (existing is not _detached_ws_transport
                    and not transport_may_use_session(session, transport)):
                return False
            # A model lease is delegated by one exact socket. Reattach retires
            # that lease and requires a new binding from the new socket.
